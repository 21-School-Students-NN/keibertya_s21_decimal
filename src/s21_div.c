...
void subtract_mantissas(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    uint64_t diff;
    uint32_t borrow = 0;
...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    ...
    if (!_is_zero(remainder) && result_scale < MAX_SCALE) {
        ...
        while (!_is_zero(temp_remainder) && (precision + result_scale) < MAX_SCALE && precision < MAX_PRECISION) {
            ...
            if (divide_mantissas(temp_remainder, value_2, &new_quotient, &new_remainder) != 0) {
                break;
            }
