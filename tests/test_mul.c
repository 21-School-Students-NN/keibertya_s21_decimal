...
START_TEST(test_mul_infinity) {
  ...
  ck_assert_int_eq(s21_mul(max_dec, val, &result), S21_TOO_LARGE);

  // MAX_DECIMAL * (-2) -> S21_NEGATIVE_INFINITY
  s21_decimal_from_int(2, 1, 0, &val);
  ck_assert_int_eq(s21_mul(max_dec, val, &result), S21_TOO_SMALL);
}
END_TEST
#endif
...
Suite* s21_mul_suite() {
  ...
  tcase_add_test(tc_extended, test_mul_positive_by_zero);
  tcase_add_test(tc_extended, test_mul_zero_by_negative);
  tcase_add_test(tc_extended, test_mul_one);
  tcase_add_test(tc_extended, test_mul_signs);
  tcase_add_test(tc_extended, test_mul_with_scale);
  tcase_add_test(tc_extended, test_mul_overflow_and_normalization_simple);
  tcase_add_test(tc_extended, test_mul_overflow_and_clean_normalization);
  tcase_add_test(tc_extended, test_mul_rounding_bankers);
  tcase_add_test(tc_extended, test_mul_rounding_simple);
  tcase_add_test(tc_extended, test_mul_infinity);
