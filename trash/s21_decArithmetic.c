...
uByte naive_addition_with_result_processing(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
...
uByte multiply_96_by_10(s21_decimal *num) {
    ...
    uInt x2[3];
    carry = (uLong)x[0] << 1; x2[0] = (uInt)carry;
...
uByte devide_96_by_10(s21_decimal *num, uInt *carry_in) {
    ...
    if(*carry_in) {tmp_num.bits[3] = tmp_info_byte;}
    SET_SCALE(&tmp_num, GET_SCALE(tmp_num)-1);
    if(!error_fl) {
        *num = tmp_num;
